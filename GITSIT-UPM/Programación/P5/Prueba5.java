...
public class Prueba5 {
	...
	public boolean assertListEquals (String msg, List<Satelite> expected, List<Satelite> obtained) {
		if (expected.size() != obtained.size()) {
			System.out.println(msg + " expected: " + expected.size() + " obtained: " + obtained.size());
			return false;
		}
		for (int i = 0; i < expected.size(); i++)
			if ((expected.get(i) != null) && (obtained.get(i) != null) && (! expected.get(i).equals(obtained.get(i)))) {
				System.out.println(msg + "[ " + i + "] expected: " + expected.get(i) + " obtained: " + obtained.get(i));
				return false;
			}
		System.out.println(msg + " ok");
		return true;
	...
	public void test1() {
		...
		assertEquals(t+i+";new EstacionSeguimiento(e1);getSatelites().length", 0, e.getSatelites().size());
		i++;
	}
	...
	public void test2() {
		...
		e.addSatelite(s1);
		assertListEquals(t+i+";new EstacionSeguimiento(e1);addSatelite(s1);", Arrays.asList(ss), e.getSatelites());
		...
		assertListEquals(t+i+";new EstacionSeguimiento(e1);addSatelite(array completo)", Arrays.asList(ss), e.getSatelites());
		...
		assertListEquals(t+i+";new EstacionSeguimiento(e1);addSatelite(más de los que caben)", Arrays.asList(ss), e.getSatelites());		
	}
	...
	public void test3() {
		...
		EstacionSeguimiento e = new EstacionSeguimiento("e1");
		List<Satelite> ss = e.getSatelitesValidos(null);
		assertListEquals(t+i+";new EstacionSeguimiento(e1);getSatelitesValidos()", Arrays.asList(new Satelite[0]), e.getSatelitesValidos(null));
		
		i++;
		// getSatelitesValidos algunos
		e = new EstacionSeguimiento("e1");
		for (int j = 0; j < 16; j++) {
			Satelite s = new Satelite ("s" + i, new Posicion(i, i, i), 0, new Posicion(0, 0, 0), 0);
			ss.add(s);
			e.addSatelite(s);
		}
		assertListEquals(t+i+";new EstacionSeguimiento(e1);addSatelite(s1);array a medias;getSatelitesValidos()", ss, e.getSatelitesValidos(null));
	}	
	...
	public void test4() {
		...
		EstacionSeguimiento e = new EstacionSeguimiento("e1");
		assertListEquals(t+i+";new EstacionSeguimiento(e1);getSatelitesValidos()", Arrays.asList(new Satelite[0]),
				e.getSatelitesValidos(new SelectorSateliteCompatible(new Posicion(0,0,0))));
		...
		assertListEquals(t+i+";new EstacionSeguimiento(e1);addSatelite(s1);array a medias;getSatelitesCompatibles(ninguno)", Arrays.asList(new Satelite [0]),
				e.getSatelitesValidos(new SelectorSateliteCompatible(new Posicion(-100,-100,-100))));
		...
		assertListEquals(t+i+";new EstacionSeguimiento(e1);addSatelite(s1);array a medias;getSatelitesCompatibles(algunos)", Arrays.asList(new Satelite []{ss[4], ss[5]}),
				e.getSatelitesValidos(new SelectorSateliteCompatible(new Posicion(5,5,5))));
	}	
	...
	public void test5() {
		...
		e.addDeteccion(p, 0); 
		assertEquals(t+i+";new EstacionSeguimiento(e1);lleno;addDeteccion(3000,3000,3000)", p, e.getSatelitesValidos(null).get(6).getPos());
		
		i++;
		// addDeteccion nuevo sin sitio
		e = new EstacionSeguimiento("e3");
		ss = new Satelite[15];
		for (int j = 0; j < ss.length; j++) {
			ss[j] = new Satelite ("s" + j, new Posicion(j*10, j*10, j*10), 0, new Posicion(0, 0, 0), 0);
			e.addSatelite(ss[j]);
		...
		p = new Posicion(3000, 3000, 3000);
		e.addDeteccion(p, 0); 
		assertEquals(t+i+";new EstacionSeguimiento(e1);lleno;addDeteccion(3000,3000,3000)", p, e.getSatelitesValidos(null).get(15).getPos());
	}
	...
	public void test6() {
		...
		EstacionSeguimiento e = new EstacionSeguimiento("e1");
		e.actualizar(10); 
		assertEquals(t+i+";new EstacionSeguimieno(e1);vacía;actualizar(10);", 0, e.getSatelitesValidos(null).size());
		...
		e.actualizar(20); 
		for (int j = 0; j < e.getSatelitesValidos(null).size(); j++) {
			assertEquals(t+i+";new EstacionSeguimiento(e1);llena;actualizar(20);", 20, e.getSatelitesValidos(null).get(j).getT());
		}
	}
