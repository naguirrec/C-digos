...
public class MapaPanel extends JPanel {
	...
	public void init(EstacionSeguimiento e, List<Deteccion> detecciones, SelectorSatelite s, long tini, long tfin, long step) {
		this.init(e, detecciones);
		this.tini = tini;
		this.tfin = tfin;
		this.step = step;
		this.t = tini;
		this.s = s;
		if (this.s == null)
			this.s = new SelectorSateliteTrue();
		//this.nDet = 0;
	}
	
    public void init(EstacionSeguimiento e, List<Deteccion> Deteccions) {
    	this.estacion = e;
    	this.detecciones = Deteccions;
    	this.s = new SelectorSateliteTrue();
    }
    ...
    public void step() {
    	...
    	this.t += this.step;
 		...
 		if (this.detecciones != null) {
 			for (Deteccion d: this.detecciones) {
 				if (d != null && d.getT() == this.t) 
 					this.estacion.addDeteccion(d.getPos(), d.getT());
 			}
 			
 		}
 		this.estacion.actualizar(this.t);
 		this.repaint();    	
    }	
	...
	public void paintComponent(Graphics g) {
		...
		for (Satelite t: this.estacion.getSatelitesValidos(s)) {
			if (t != null) {
				this.paintSatelite (g, t, false);
			}
		}
		for (Deteccion t: this.detecciones) {
			if ((t != null) && (t.getT() <= this.t))
				this.paintDeteccion (g, t);
		}
	}
