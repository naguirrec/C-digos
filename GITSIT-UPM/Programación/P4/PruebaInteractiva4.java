...
public class PruebaInteractiva4 {
	...
	public String run (String c) {
		...
		if (c.indexOf(cmds[18].split(" ")[0]) == 0) { // "compatible"
			SelectorSatelite ss = new SelectorSateliteEnPeligro(this.estacion.getSatelitesValidos(null));
			this.s2 = this.s1;
			this.s1 = ss;
			return this.s1.toString();
		}
		...
		if (c.indexOf(cmds[20].split(" ")[0]) == 0) { // "show"
			...
			SwingUtilities.invokeLater(new Runnable() {
	            public void run() {
	                ...
	                pva.init(estacion, detecciones, s1, 0, 0, 0); 
